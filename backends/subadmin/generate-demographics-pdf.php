...
try {
  ...
  // Fetch business name
  $businessQuery = "SELECT BusinessName FROM businessinformationform WHERE ApplicationID = :applicationID LIMIT 1";
  $businessStmt = $pdo->prepare($businessQuery);
  $businessStmt->execute(['applicationID' => $applicationID]);
  ...
  $pdf->SetFont('helvetica', '', 7); // Adjust font size
  $pdf->Cell(20, 10, '', 0, 0); // Add an empty cell for spacing (20 units)
  $pdf->Cell(0, 0, 'Name of Municipality: Majayjay', 0, 1, 'L'); // No margin top

  $pdf->SetFont('helvetica', '', 7); // Adjust font size
  $pdf->Cell(20, 10, '', 0, 0); // Add an empty cell for spacing (20 units)
  $pdf->Cell(0, 0, 'Name of attraction/ Spot: ' . $businessName, 0, 1, 'L'); // No margin top
